...
class LLMEvalAdapter:
    ...
    def _get_or_create_async_limiter(
        self, model: str
    ) -> AsyncConcurrencyLimiter | None:
    ...
    async def agenerate(
        ...
        if (
            ...
            and thinking_cfg.get("request_params")
        ):
    ...
    async def _single_api_request(
        ...
        async with client.stream("POST", url, json=payload, headers=headers) as resp:
            ...
            async for line in resp.aiter_lines():
                ...
                now = time.monotonic()
                if chunk_count > 1 and now - t_last_activity > _CHUNK_IDLE_TIMEOUT:
                    raise TimeoutError(
                ...
                if chunk.get("type") == "error":
                    ...
                    logger.error(f"[{model}] {err_msg}, data: {data_str[:300]}")
                ...
                if choices:
                    ...
                    if reasoning_field_auto_detected and chunk_count <= 2:
                        for k, v in delta.items():
                            if (
                                k not in _SKIP_DELTA_KEYS
                                and v
                                and k in _REASONING_FIELD_CANDIDATES
                            ):
                    ...
                    delta_reasoning = (
                        delta.get(detected_reasoning_field)
                        if detected_reasoning_field
                        else None
                    )
        ...
        if not got_done:
            logger.debug(f"[{model}] 流未收到结束标记，但已有内容，视为正常结束")
        ...
        if not full_content.strip():
            ...
            if chunk_count == 0:
                raise ConnectionError(f"服务端未返回任何 SSE 数据 (model={model})")
            if comp_tokens > 0:
                raise ConnectionError(
                    f"token 计数非零但内容为空 "
        ...
        return GenerateResponse(
            ...
            ttft=ttft,
            ttft_content=t_first_content_token - t_start
